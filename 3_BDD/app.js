...
const API_URL = '/3_BDD/api/search';
...
function renderResults(data) {
  const { results, total, page, pageSize, hasMore, confidence, message } = data;
  ...
  if (total === 0) {
    ...
    els.resultsHeader.textContent =
      ...
      `para "${state.query}" — exibindo ${from}–${to}`;
  }
  ...
  if (results.length === 0) {
    els.resultsList.innerHTML =
      `<li class="no-results"><strong>🎞️</strong>${message || 'Nenhum resultado encontrado para sua busca.'}</li>`;
  } else {
...
function movieCard(m) {
  ...
  const relWidth   = Math.max(4, relPct); // always show at least a sliver
...
function buildStars(rating) {
  const full  = Math.round(rating / 2);       // 0-5 full stars
  const empty = 5 - full;
